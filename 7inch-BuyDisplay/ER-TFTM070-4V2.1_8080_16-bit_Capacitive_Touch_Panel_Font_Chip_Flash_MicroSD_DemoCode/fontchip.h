...
sbit GTCS        =   	P2^3;    //  Font  Chip enable input
...
//=================================================================
//从DataAddress这个地址逐个读出DataLen个字节的数据到 pBuff的地址
//逐个读取
//=================================================================
//-------------------------------------------------------------------------------
...
{uchar code text[]={"深圳旭日东方科技有限公司"};
	uchar code 	text1[]={" ER-TFT070-4V2.1  800RGB*480 DOTS"};
		uchar code 	text2[]={"EAST RISING  WWW.BUYDISPLSY.COM"};
	uchar i=0; uint x=150,y=30;
  	unsigned long fontaddr;	

	while(text[i]>0x00)
	{
			fontaddr = GB2312_Addr(text[i],text[i+1]);  				 //得到　” 点阵数据的起始地址
			GT_Read_1_n(fontaddr,fontbuf,32 );							  		//得到　” 点阵数据			
			Display_One_Chinese_16X16(y,x,fontbuf,0xf800,0x0000);			   //在坐标（X，Y），显示汉字“　?
			i+=2;
			x+=16;
	...
	while(text1[i]>0x00)
	{
				fontaddr = GB2312_Addr(text1[i],0);  				 //得到　” 点阵数据的起始地址
				GT_Read_1_n(fontaddr,fontbuf,16 );							  		//得到　” 点阵数据	
   				Display_One_Chinese_8X16(y,x,fontbuf,0xf800,0xffff);			   //在坐标（X，Y），显示汉字“　?
			i+=1;
			x+=8;
	}
	...
	while(text2[i]>0x00)
	{
				fontaddr = GB2312_Addr(text2[i],0);  				 //得到　” 点阵数据的起始地址
				GT_Read_1_n(fontaddr,fontbuf,16 );							  		//得到　” 点阵数据	
   				Display_One_Chinese_8X16(y,x,fontbuf,0xf800,0x001f);			   //在坐标（X，Y），显示汉字“　?
...
uint i= 0; uint x=0,y=0;    uchar k,k1;  
	...
	while((text[i])>0x00)
	{
		...
		{	
				...
				fontaddr = GB2312_Addr(text[i],0);  				 //得到　” 点阵数据的起始地址
				GT_Read_1_n(fontaddr,fontbuf,16 );							  		//得到　” 点阵数据	
   				Display_One_Chinese_8X16(y,x,fontbuf,0xf800,0x0000);			   //在坐标（X，Y），显示汉字“　?
